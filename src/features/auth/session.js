 ...
 */
export async function loadSessionRoles(userId, email) {
    ...
    return {
        ...
        organizations: (orgMemberships.data ?? []).map((m) => ({
            ...
            role: m.role,
            displayName: m.organizations?.display_name ?? 'Organización',
        })),
...
export function resolvePersona(roles) {
    if (!roles)
        return 'UNKNOWN';
    if (roles.platformRole)
        return 'EBIM';
    if (roles.organizations.length > 0)
        return 'PARTNER';
    if (roles.salesAgentId)
        return 'SALES_AGENT';
    if (roles.tenantRoles.length > 0)
        return 'TENANT';
    return 'UNKNOWN';
}
export function isFinance(roles) {
    return roles?.platformRole === 'EBIM_FINANCE' || roles?.platformRole === 'EBIM_SUPER_ADMIN';
}
export function canManagePlatform(roles) {
    return (roles?.platformRole === 'EBIM_PRODUCT_ADMIN' || roles?.platformRole === 'EBIM_SUPER_ADMIN');
}
/**
 ...
 */
export const CONSOLE_ROLES = ['EBIM_SUPER_ADMIN', 'EBIM_PRODUCT_ADMIN', 'EBIM_FINANCE'];
/**
 * Contrato §13.2: el dominio operador ÚNICO es `ebim.pe`.
 * `grupoebim.com` es un dominio de negocio normal — NO va en esta lista.
 */
export const BLOCKED_OPERATOR_DOMAINS = ['ebim.pe'];
export function isOperatorDomain(email) {
    const domain = email.toLowerCase().split('@')[1] ?? '';
    return BLOCKED_OPERATOR_DOMAINS.includes(domain);
}

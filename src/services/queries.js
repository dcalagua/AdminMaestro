...
export function useProducts() {
    return useQuery({
        ...
        queryFn: async () => unwrap(await supabase.from('saas_products').select('*').order('sort_order')),
    });
}
...
export function useOrganizationAgreements(organizationId) {
    return useQuery({
...
export function useTenantOverview() {
    return useQuery({
        ...
        queryFn: async () => unwrap(await supabase.from('v_tenant_overview').select('*').order('name')),
    });
}
...
export function useTenantFeatures(tenantId) {
    return useQuery({
...
export function useSubscriptions() {
    return useQuery({
        ...
        queryFn: async () => unwrap(await supabase
            .from('subscriptions')
            .select('*, saas_products(code, short_name), plans(name), organizations!subscriptions_billed_organization_id_fkey(display_name), tenants(name, slug, deployment_mode), subscription_items(*)')
            .order('code')),
    });
}
...
export function useAttributions() {
    return useQuery({
        ...
        queryFn: async () => unwrap(await supabase
            ...
            .select('*, sales_agents(full_name, code, agent_type), saas_products(short_name, code), tenants(name, slug), organizations!sales_attributions_customer_organization_id_fkey(display_name), commission_plans(name, code)')
...
export function useCommissionEvents() {
    return useQuery({
        ...
        queryFn: async () => unwrap(await supabase
            ...
            .select('*, sales_agents(full_name, code), saas_products(short_name), tenants(name), commission_rules(name, basis), commission_settlements(code, status)')
...
export function useDeploymentTargets() {
    return useQuery({
        ...
        queryFn: async () => unwrap(await supabase
            ...
            .select('*, organizations(display_name), saas_products(short_name), tenant_deployments(tenant_id, status, tenants(name, slug))')
            .order('code')),
    });
}
...
export function useTenantAttributions(tenantId) {
    return useQuery({

# Fase 12 - Comisiones integradas a cobros reales
...
Extender el motor de comisión sin romper la regla fundamental: solo dinero confirmado devenga.

## Casos

1. Culqi cobro recurrente confirmado -> payment CONFIRMED -> comisión.
2. Transferencia/Manual confirmada por usuario finance autorizado -> payment CONFIRMED -> comisión.
3. SERVICE_ORDER/PO recibida o aprobada -> **NO comisión** hasta payment CONFIRMED.
4. Implementation Fee puede ser comisionable solo si la commission_rule usa `COLLECTED_IMPLEMENTATION` o `COLLECTED_ANY` y el fee fue cobrado.
5. Partner margin y sales agent commission son conceptos distintos; no restar dos veces.
6. Reversal/refund debe anular/compensar comisión de manera explícita y auditable; no borrar eventos históricos.

## Cambios

Inspecciona `generate_commission_events()` y amplíala solo donde haga falta para provider/reversal, conservando idempotencia.

Si agregas reversal, define mecanismo contable de contra-evento o estado VOID con snapshot que preserve historial. Documenta la decisión.

## UI

En detalle de pago mostrar:
- factura;
- método/proveedor;
- reference externa;
...
En Comisiones mostrar fuente: licencia, implementación, etc.

## Gate

Un webhook repetido 5 veces genera 1 payment lógico y 1 conjunto de commission_events, no 5.

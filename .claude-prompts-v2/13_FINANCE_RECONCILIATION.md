# Fase 13 - Finanzas, reconciliación y margen
...
Dar a gerencia una vista confiable de facturado, cobrado, comisiones, costos y margen por SaaS/partner/tenant.

## Requisitos

1. Conserva la semántica actual: margen calculado sobre cobrado cuando el reporte lo indique, no sobre factura DRAFT.
2. Añade reconciliación:
   - provider payment vs local payment;
   - payments sin invoice;
   - invoices abiertas;
   - provider subscriptions desincronizadas;
   - OS/OC vencidas;
   - payment failures;
   - reversos.
3. Vista/UI `Reconciliación` con estados `OK`, `REVIEW`, `ERROR`.
4. Dashboard gerencial por producto:
   - MRR;
   - one-time implementation cobrado;
   - infraestructura cobrada;
   - cobrado total;
...
5. Partner:
   - revenue atribuible;
   - margen partner;
   - comisión adicional a sales agents si aplica;
   - costo dedicado.
6. Tenant:
   - licencia;
   - fees;
...
No mezcles PEN y USD en un único total sin FX explícito. Si no hay tabla FX, agrupa por moneda y documenta que conversión consolidada queda fuera de alcance V2.

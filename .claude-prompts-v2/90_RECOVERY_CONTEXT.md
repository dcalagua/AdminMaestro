# RECOVERY - Reanudar el trabajo en otro chat o después de compactación
...
1. NO reinicies el proyecto ni repitas fases a ciegas.
2. Lee:
   - `.claude-prompts-v2/STATE_V2.md`
   - `.claude-prompts-v2/DECISIONS_V2.md`
   - `.claude-prompts-v2/QUALITY_GATE_V2.md`
   - `docs/nightly-v2/GAP_MATRIX.md` si existe
   - `docs/nightly-v2/FINAL_REPORT_V2.md` si existe
   - `git status` y últimos commits.
3. Lista las migraciones existentes y confirma cuál es la última.
4. Identifica la primera fase V2 que NO está `COMPLETED/PASS`.
5. Lee el archivo de esa fase y continúa desde la primera acción pendiente.
6. Si una fase decía PASS pero el código actual no lo respalda, cambia el estado a FAIL/REGRESSION y corrígelo antes de avanzar.
7. Nunca uses este recovery como justificación para saltar la auditoría final.

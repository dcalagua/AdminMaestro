# Fase 17 - E2E y certificación funcional
...
Validar desde navegador los journeys que venderemos a gerencia.

## E2E mínimos

1. EBIM Super Admin crea partner y acuerdo eSupplier Shared.
2. EBIM crea cliente/tenant Shared directo con implementation fee.
3. Partner admin crea/gestiona un tenant Shared permitido; no accede a otro partner.
4. Partner Shared administra múltiples tenants sin target dedicado.
5. EBIM configura Partner Dedicated y encola provisioning DRY_RUN.
6. EBIM configura Tenant Dedicated Enterprise.
7. GRUPASA: dos SaaS con dos métodos de cobro distintos.
8. SERVICE_ORDER: request -> received -> approved sin crear payment.
9. Culqi MOCK/TEST: setup -> webhook/payment confirmed -> comisión; repetir webhook no duplica.
10. Comercial independiente ve su comisión pero no obtiene acceso operativo.
11. Renovación genera alerta de 45 días y estado de gracia determinista.
12. Usuario no autorizado intenta mutación y recibe denegación real.
13. Vista 360 Organization muestra productos, tenants, cobranza y margen.

## Ejecución

- `npm run e2e`
- Si Playwright necesita browsers, instala solo lo necesario dentro de las prácticas existentes.
- Captura evidencia textual en `docs/nightly-v2/E2E_REPORT.md`: nombre del test, PASS/FAIL y motivo de cualquier skip.

No aceptes skips de seguridad o billing crítico como PASS.

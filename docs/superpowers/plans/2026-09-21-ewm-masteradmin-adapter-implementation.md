...
**Base:** `dev` @ `afc40cf`. El commit `a25e4a7` (CORS) ya está en la rama y **no** se reimplementa.
...
- El despliegue de la función y la carga del secreto están autorizados por el usuario para esta ejecución. Si el harness deniega el comando, el operador lo ejecuta con `!` y el agente verifica el resultado. **Nunca** se reintenta por otra vía.
...
| Reaplicar migraciones locales | `supabase db reset` (**sólo local**) |
...
| Advisors | `supabase db advisors --local --type security --output-format json` / `--linked` |
...
| R5 · EWM responde `201` válido pero incompleto | `ewm-v1.test.ts › "201 sin companyId / sin organizationId / sin provisioningId / sin replayed → PROVIDER_RESPONSE_INVALID"`, `"controlPlaneTenantId distinto → inválido"`, `"companyId distinto del enviado → inválido"`, `"status ≠ ACTIVE → inválido"`, `"companyId no UUID → inválido"` | 8 |
...
- [ ] **3.8 Verificación:**
   ...
   - `git diff --stat afc40cf -- supabase/functions/_shared/provisioning/adapters/http-m2m.generic-golden.test.ts` → vacío;
...
- [ ] **4.1 Pruebas que fallan** (`ewm-v1.test.ts`). Fixture `ewmContext()` = contexto con `source` completo (tenant `50000000-…0008`, organización `30000000-…0004` slug `empresa-directa-alpha`, sociedad `31000000-…0001` PEN) y `product_configuration` válida.
   - **Cuerpo exacto:** `EWM_V1_CODEC.buildCreateBody(ewmContext())` `toEqual` el literal completo de `EwmCreateBody`. Comprueba en particular:
     ...
     - `admin.email = source.tenant.admin_email`;
...
- [ ] **5.5** `supabase db reset` → `npm run db:test` PASS → `npx vitest run supabase/functions/_shared/provisioning` PASS → con §0.1, E2E dorada + `v4-provisioning-orchestrator.spec.ts` PASS.
...
- [ ] **6.4** `supabase db reset` → `npm run db:test` PASS → `npm run typecheck` PASS.
...
- [ ] **7.3** `supabase db reset` → `npm run db:test` PASS → vitest provisioning PASS.
...
- [ ] **10.3 Pruebas puras que fallan** (`status.test.ts`), sobre `summarizeStatus(outcome, mapping)`:
   ...
   - falla con `httpStatus 404` → `{ found:false, remote:null, mapping_consistent: mapping === null }`;
...
- [ ] **10.5 Orquestador, rama `GET_STATUS`:**
   ...
   8. respuesta `{ request_id, found, remote, mapping_consistent, provider_http_status }`.
...
- [ ] **10.7** `supabase db reset` → `npm run db:test` → vitest → E2E dorada + orquestador PASS.
...
- [ ] **11.6** `supabase db reset` → pgTAP → vitest → E2E PASS.
...
## Task 13 — Gate local previo a cualquier cambio remoto

Con §0.1, sin excepciones:

- [ ] `supabase db reset` → `npm run db:test` → **PASS** (todos los archivos, incluidos `21`, `22` y `23`).
- [ ] `npx vitest run` → PASS; **dorada** `http-m2m.generic-golden.test.ts` PASS y `git diff afc40cf -- <dorada>` vacío (H1).
...
- [ ] Advisors locales, antes y después:
   1. **Antes:** `git worktree add "$TMPDIR/base" afc40cf`, luego `(cd "$TMPDIR/base" && supabase db reset --workdir "$TMPDIR/base")` contra el mismo stack local, y `supabase db advisors --local --type security --output-format json > "$TMPDIR/advisors-local-before.json"`.
   2. **Después:** `supabase db reset` en el repo y `supabase db advisors --local --type security --output-format json > "$TMPDIR/advisors-local-after.json"`.
...
- [ ] **14.1 Huella (H5), sin mostrar la clave:**
...
- [ ] **14.2 Conversión SEC1 → PKCS#8 → secret**, sin archivo intermedio, sin stdout de la clave y sin la clave en `argv`:
...
```
   ...
   (La clave pasa por un descriptor de proceso, `/dev/fd/N`, que no se escribe a disco. Si el harness deniega el comando, lo ejecuta el operador con `!`.)
- [ ] **14.3 Confirmar sólo el nombre:** `supabase secrets list --project-ref jivgwrczgdpsuvqcwqku | grep -o '"name":"EWM_QAS_M2M_PRIVATE_KEY"'`. La CLI de esta máquina (v2.116) imprime `{"secrets":[{"name":…,"value":<digest>}]}`; se filtra sólo el nombre.
- [ ] **14.4** No hay commit.

## Task 15 — Migración en QAS (remoto, operador humano)

- [ ] **15.1** El agente verifica: `supabase migration list --linked` → la única pendiente es `20260921000100` (H3/H12: el proyecto enlazado es `jivgwrczgdpsuvqcwqku`, «AdminMaestro»).
...
- [ ] **16.2** `supabase functions deploy provisioning-orchestrator --project-ref jivgwrczgdpsuvqcwqku`. **Sólo** esta función.
...
Por la consola local contra QAS (guardia inversa §0.1) y sus RPC oficiales, reutilizando los registros existentes **sin duplicarlos**:
...
- [ ] `configure_deployment_provisioning` sobre `40000000-0000-4000-a000-000000000008` (`ewm-shared-qas`): `base_url 'https://ewm-rsxs.onrender.com'`, `QAS`, `timeout_ms 30000`, `retry_count 2`, `DRAFT`, `enabled false`.
...
- [ ] Habilitar **en orden**, con las mismas RPC:
   ...
   3. deployment `READY` + `provisioning_enabled true` (el trigger `enforce_deployment_provisioning_coherence` debe aceptar).
...
- [ ] Crear en QAS, **por la UI local**, la organización **EBIM EWM QAS Smoke**:
   ...
   - alta EWM SHARED con Onboarding (`onboard_customer_subscription`): tenant `ebim-ewm-qas-smoke`, admin `ewm.masteradmin.smoke+<epoch>@ebim.test` (nuevo y único).
   - El `tenants.id` resultante **es** el `controlPlaneTenantId`, generado una sola vez; se anota.
- [ ] Provisioning SaaS → «Nueva solicitud» (`ProvisioningDialogs` → `create_saas_provisioning_request`) → `READY_TO_PROVISION`.
...
- [ ] «Consultar estado» (GET_STATUS) → esperado `found:false`, `provider_http_status 404` (`RESOURCE_NOT_FOUND`). **Significa que EWM validó la firma ES256, el `iss`, el `aud` y el scope `ewm:tenant:read`.** `401` → H7. `403` → scope: H7. `PRIVATE_KEY_INVALID` o `SECRET_NOT_AVAILABLE` → volver a la Task 14.
...
- [ ] `EWM_QAS_CERTIFICATION=1 REQUEST_ID=<id> npx playwright test e2e/v4-ewm-qas-certification.spec.ts -g replay`:
...
- [ ] `… -g status`, o «Consultar estado» en la UI → `found:true`, `provider_http_status:200`, `mapping_consistent:true` (H10), ruta `/internal/platform/v1/tenants/<controlPlaneTenantId>`.
...
- [ ] Fuera de los logs: `supabase functions logs`, si está disponible en la CLI; si no, el panel de logs de la función, filtrado por `PRIVATE KEY|eyJhbGciOi` → vacío. Se deja constancia del método usado.
...
13. `test(e2e): add opt-in EWM QAS certification` (el spec de Playwright de las Tasks 21–22, escrito y probado en seco en la Task 13 con el guard de entorno)
14. `docs(provisioning): document EWM compatibility adapter`
...
| Control | Resultado |
| --- | --- |
| Cobertura de la spec §1–§26 | §7–8 → Tasks 1 y 3; §9–10 → 4, 6 y 7; §11 → 8; §12 → 10; §13 → 9; §14 → 2, 5, 6 y 7; §15 → 11; §16 → 3, 5 y 10; §17 → 2, 5, 6, 7, 10 y 11; §18 → 12; §19 → 23; §20 → 6, 10 y 11; §21 → todas las de prueba; §22 → orden de commits; §23 → 14–22; §24 → rollback de la spec (sin cambios); §25 → 13, 24 y 20–22; §26 → File Map |
...
| Nada remoto antes de los gates | Task 13 precede a 14–22 |
